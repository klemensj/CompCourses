...
def is_DNA_checker(DNA):
    for i in range(len(DNA)):
        if DNA [i] == "A"or"C"or"G"or"T":
            print("valid_DNA")
        else:
            print("Invalid_DNA")  
                          # What if statement would allow this code to work to 
                         # check whether or not a particular string represented
                         # DNA?
...
def NucleotideCounter(DNA):
    ...
    for i in range(len(DNA)):      # define a for loop that inspects each element
        if DNA [i] == "A":
             A += 1
        if DNA [i] == "C":
             C += 1
        if DNA [i] == "G":
             G += 1
        if DNA [i] == "T":
             T += 1        # in DNAstring, use an if ... elif ... 
                            #A += 1
                            #elif 
                            #  ...
                             #
...
def find_start_codon(mRNA):
    ...
    for i in range(len(mRNA)):                 # hint for ? below: we want i to be a number
        if mRNA[i:i+3] == "AUG":               # keep the loop going until we find the first
            ...
            return("valid_DNA")                        # what should we return here?
